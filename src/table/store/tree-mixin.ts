...
export default class Tree {
	onReady() {
		 ...
		 */
		this.normalizedData = computed(() => {
			if (!this.states.rowKey) return {};

			return this.normalize(this.states.data || []);
		});
		/**
		 * 针对懒加载的情形，不处理嵌套数据
		 * @return { id: { children } }
		 */
		this.normalizedLazyNode = computed(() => {
		...
		watch(
			...
			this.updateTreeData.bind(this),
		);
		watch(
			() => this.normalizedLazyNode.value,
			this.updateTreeData.bind(this),
		);
	}
	...
	toggleTreeExpansion(row, expanded) {
		...
		if (id && data && 'expanded' in data) {
			...
			this.states.treeData[id].expanded = expanded;
	...
	loadData(row, key, treeNode) {
		...
		if (this.table.props.loadExpand && !treeData[key].loaded) {
			
			this.states.treeData[key].loading = true;
			let promise = this.table.props.loadExpand(row, treeNode);
			let fn = data => {
				if (!Array.isArray(data)) {
					throw new VcError('table', 'data必须是数组');
				}
				this.states.treeData[key].loading = false;
				this.states.treeData[key].loaded = true;
				this.states.treeData[key].expanded = true;
				...
				if (data.length) {
					this.states.lazyTreeNodeMap[key] = data;
