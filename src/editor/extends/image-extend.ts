...
class ImageExtend {
	...
	initUploadInstance = () => {
		...
		this.vm = createApp(() => h(Upload, {
			ref: i => i && (this.vm.wrapper = i),
			onFileStart: this.handleUploadStart,
			onFileSuccess: this.handleUploadSuccess,
	...
	readFiles = (files) => {
		[].forEach.call(files, file => {
			...
			this.vm.wrapper.uploadFiles([file]);
		});
	}
